...
const StarRating = ({ value }) => {
  ...
  return <span style={{ color: "#FFD600" }}>{"★".repeat(stars)}{"☆".repeat(5 - stars)}</span>;
};

const StatCard = ({ label, value, sub, color }) => (
  <div style={{ background: CARD2, border: `1px solid ${color || BORDER}`, borderRadius: 16, padding: "18px 20px", flex: "1 1 140px", minWidth: 130, boxShadow: color ? `0 0 18px ${color}22` : "none" }}>
    <div style={{ fontSize: 11, color: "#888", letterSpacing: 2, textTransform: "uppercase", marginBottom: 6, fontFamily: "monospace" }}>{label}</div>
    <div style={{ fontSize: 28, fontWeight: 900, color: color || "#fff", fontFamily: "'Bebas Neue','Impact',sans-serif", lineHeight: 1 }}>{value}</div>
...
const MiniBar = ({ pct, color }) => (
  <div style={{ background: "#1a1a26", borderRadius: 4, height: 6, width: "100%", marginTop: 4 }}>
    <div style={{ width: `${Math.min(100, pct || 0)}%`, height: "100%", background: color, borderRadius: 4, transition: "width 0.6s ease" }} />
  </div>
);
...
export default function App() {
    ...
    reader.onload = (e) => {
      try {
        ...
        const cm = {
          ...
          pronostico: findCol(hdrs, ["pronóstico","pronostico","prediccion","predicción","pick","tip","apuesta"]),
          cuota: findCol(hdrs, ["cuota","odd","quota","momio"]),
          confianza: findCol(hdrs, ["confianza","confidence","conf","%","certeza"]),
          fecha: findCol(hdrs, ["fecha","date","día","dia"]),
  ...
  const leagues = colMap.liga ? ["all", ...new Set(predictions.map(r => r[colMap.liga]).filter(Boolean))] : ["all"];
  const filtered = predictions.filter(r => {
    const matchText = filter ? Object.values(r).some(v => normalize(v).includes(normalize(filter))) : true;
    const matchLeague = filterLeague !== "all" && colMap.liga ? normalize(r[colMap.liga]) === normalize(filterLeague) : true;
  ...
  const s = {
    app: { minHeight: "100vh", background: BG, color: "#fff", fontFamily: "'DM Sans','Segoe UI',sans-serif", paddingBottom: 80 },
    header: { background: "#0d0d18", borderBottom: `1px solid ${BORDER}`, padding: "16px 20px", position: "sticky", top: 0, zIndex: 100 },
    logo: { fontSize: 22, fontWeight: 900, fontFamily: "'Bebas Neue','Impact',sans-serif", letterSpacing: 3, background: `linear-gradient(90deg,${ACCENT},${ACCENT2})`, WebkitBackgroundClip: "text", WebkitTextFillColor: "transparent" },
    nav: { display: "flex", gap: 6, background: CARD, borderRadius: 14, padding: 4, margin: "16px 16px 0" },
    navBtn: (active) => ({ flex: 1, padding: "10px 0", borderRadius: 10, border: "none", cursor: "pointer", fontWeight: 700, fontSize: 13, background: active ? `${ACCENT}22` : "transparent", color: active ? ACCENT : "#666", borderBottom: active ? `2px solid ${ACCENT}` : "2px solid transparent" }),
    section: { padding: "16px" },
    card: { background: CARD, border: `1px solid ${BORDER}`, borderRadius: 16, padding: 16, marginBottom: 12 },
    badge: (color) => ({ display: "inline-block", padding: "3px 10px", borderRadius: 20, fontSize: 11, fontWeight: 700, background: `${color}22`, color, border: `1px solid ${color}44` }),
    input: { width: "100%", background: CARD2, border: `1px solid ${BORDER}`, borderRadius: 10, padding: "10px 14px", color: "#fff", fontSize: 14, outline: "none", boxSizing: "border-box" },
  };
    ...
    <div style={s.app}>
      ...
      <div style={s.section}>
        <div onDragOver={(e) => { e.preventDefault(); setDragging(true); }} onDragLeave={() => setDragging(false)} onDrop={onDrop} onClick={() => fileRef.current.click()}
          style={{ border: `2px dashed ${dragging ? ACCENT : BORDER}`, borderRadius: 20, padding: "48px 24px", textAlign: "center", cursor: "pointer", background: dragging ? `${ACCENT}08` : CARD }}>
          <div style={{ fontSize: 48, marginBottom: 12 }}>📊</div>
          <div style={{ fontSize: 18, fontWeight: 700, color: ACCENT, marginBottom: 8 }}>Sube tu Excel de predicciones</div>
          <div style={{ fontSize: 13, color: "#666", marginBottom: 16 }}>Toca para seleccionar el archivo</div>
          <div style={{ fontSize: 11, color: "#444", fontFamily: "monospace" }}>.xlsx · .xls · .csv</div>
        ...
        {error && <div style={{ marginTop: 12, padding: "10px 14px", background: "#FF444422", border: "1px solid #FF4444", borderRadius: 10, color: "#FF4444", fontSize: 13 }}>{error}</div>}
        <div style={{ ...s.card, marginTop: 16 }}>
          <div style={{ fontSize: 12, color: "#888", marginBottom: 10, fontWeight: 700, letterSpacing: 1, textTransform: "uppercase" }}>Columnas que reconoce</div>
          {[["Partido","match, juego"],["Liga","league, torneo"],["Pronóstico","pronostico, pick"],["Cuota","odd, momio"],["Confianza","confidence, %"],["Fecha","date"],["Resultado","result, win/loss"],["Mercado","market, tipo"]].map(([col, aliases]) => (
            <div key={col} style={{ display: "flex", justifyContent: "space-between", padding: "6px 0", borderBottom: `1px solid ${BORDER}`, fontSize: 13 }}>
    ...
    <div style={s.app}>
      ...
      <div style={s.header}>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
          <div style={s.logo}>⚽ PRONO PRO</div>
          <button onClick={() => { setPredictions([]); setFileName(""); }} style={{ background: "#FF444422", border: "1px solid #FF444444", color: "#FF4444", borderRadius: 8, padding: "6px 12px", fontSize: 12, cursor: "pointer", fontWeight: 700 }}>↩ Cambiar</button>
        </div>
        <div style={{ fontSize: 11, color: "#555", marginTop: 4, fontFamily: "monospace" }}>📂 {fileName} · {total} predicciones</div>
      </div>
      ...
      {tab === "dashboard" && (
        <div style={s.section}>
          ...
          <div style={{ display: "flex", flexWrap: "wrap", gap: 10, marginBottom: 16 }}>
            ...
            <StatCard label="Cuota avg" value={avgCuota || "—"} color="#00C9FF" />
            {profit !== null && <StatCard label="Profit (u)" value={`${profit > 0 ? "+" : ""}${profit}`} color={parseFloat(profit) > 0 ? ACCENT : "#FF4444"} sub="unidades" />}
          </div>
          {withResult.length > 0 && (
            <div style={s.card}>
              <div style={{ fontSize: 12, color: "#888", marginBottom: 12, fontWeight: 700, letterSpacing: 1, textTransform: "uppercase" }}>Rendimiento global</div>
          ...
          {colMap.confianza && (
            <div style={s.card}>
              <div style={{ fontSize: 12, color: "#888", marginBottom: 12, fontWeight: 700, letterSpacing: 1, textTransform: "uppercase" }}>Distribución confianza</div>
              {[["Alta 80-100%",80,101,ACCENT],["Media-Alta 60-79%",60,80,"#FFD600"],["Media 40-59%",40,60,"#FF9800"],["Baja menos 40%",0,40,"#FF4444"]].map(([label,min,max,color]) => {
          ...
          {colMap.confianza && (
            <div style={s.card}>
              ...
              {[...predictions].sort((a,b) => (parseFloat(b[colMap.confianza])||0)-(parseFloat(a[colMap.confianza])||0)).slice(0,5).map((r,i) => (
                <div key={i} style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "8px 0", borderBottom: `1px solid ${BORDER}` }}>
                  <div>
                    <div style={{ fontSize: 13, fontWeight: 600 }}>{r[colMap.partido] || "—"}</div>
      ...
      {tab === "predictions" && (
        <div style={s.section}>
          <input style={{ ...s.input, marginBottom: 12 }} placeholder="🔍 Buscar partido, equipo..." value={filter} onChange={e => setFilter(e.target.value)} />
          {colMap.liga && (
            <div style={{ marginBottom: 12, overflowX: "auto", display: "flex", gap: 6, paddingBottom: 4 }}>
              {leagues.map(l => (
                <button key={l} onClick={() => setFilterLeague(l)} style={{ ...s.badge(filterLeague === l ? ACCENT : "#555"), cursor: "pointer", whiteSpace: "nowrap", fontSize: 12, padding: "5px 12px" }}>
                  {l === "all" ? "Todas" : l}
                </button>
              ))}
            </div>
          ...
          {filtered.map((row, i) => {
            ...
            return (
              <div key={i} style={{ ...s.card, borderLeft: `3px solid ${resColor}`, marginBottom: 10 }}>
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 8 }}>
                  <div style={{ flex: 1 }}>
                    <div style={{ fontSize: 15, fontWeight: 700, marginBottom: 4 }}>{row[colMap.partido] || `Pick #${i+1}`}</div>
                    <div style={{ display: "flex", gap: 6, flexWrap: "wrap" }}>
                      {colMap.liga && row[colMap.liga] && <span style={s.badge(ACCENT2)}>{row[colMap.liga]}</span>}
                      {colMap.mercado && row[colMap.mercado] && <span style={s.badge("#888")}>{row[colMap.mercado]}</span>}
                      {colMap.fecha && row[colMap.fecha] && <span style={{ fontSize: 11, color: "#555" }}>📅 {row[colMap.fecha]}</span>}
                    </div>
                  </div>
                  {res && <span style={{ ...s.badge(resColor), fontSize: 13, fontWeight: 900 }}>{res === "win" ? "✓ W" : res === "loss" ? "✗ L" : "= E"}</span>}
                </div>
      ...
      {tab === "leagues" && (
        <div style={s.section}>
          {leagueStats.length === 0
            ? <div style={{ textAlign: "center", color: "#555", padding: 40 }}>No se detectó columna de Liga</div>
                ...
                return (
                  <div key={liga} style={s.card}>
                    ...
                    <div style={{ display: "flex", gap: 10, marginBottom: 8 }}>
                      <span style={s.badge(ACCENT)}>✓ {st.wins}W</span>
                      <span style={s.badge("#FF4444")}>✗ {st.losses}L</span>
                      {st.total - st.wins - st.losses > 0 && <span style={s.badge("#888")}>⏳ {st.total - st.wins - st.losses}</span>}
                    </div>
                    {st.wins + st.losses > 0 && (
                      <>
